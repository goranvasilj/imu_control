...
def up(scale):
    ...
    global left_wrist, yaw_command
    x=(right_wrist.transform.translation.y)-0.1
    x=calculate_based_on_dead_zone(x, dead_zone)
    print "up ",x
    current_pose.position.z = pose_stamped.pose.position.z+x/scale
    
def yaw(scale):
    global right_wrist, yaw_command
    x=calculate_based_on_dead_zone(right_wrist.transform.translation.z, dead_zone)

    yaw_command=yaw_command+x/15/scale
    print "yaw ", x
...
def side(scale):
    global left_wrist, yaw_command
    x=calculate_based_on_dead_zone(left_wrist.transform.translation.z+0.25, dead_zone)
    print "side ",x

    current_pose.position.x = current_pose.position.x + sin(yaw_command) * x*2/scale
    current_pose.position.y = current_pose.position.y + cos(yaw_command) * x*2/scale
...
def forward(scale):
    ...
    x=(left_wrist.transform.translation.y)-0.15
    x=calculate_based_on_dead_zone(x, dead_zone)
    print "forward ",x
